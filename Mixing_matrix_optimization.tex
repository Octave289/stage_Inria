...
using a time-discretized dynamical model (to be detailed in Section~\ref{sec:model}). 
...
T = \text{[TO COMPLETE]},
\]
and the flow velocity is set to:
\[
...
the optimal permutations transport biomass toward layers with higher growth rates.

More generally, the optimal permutation matrices tend to concentrate biomass 
in the layers maximizing the local growth coefficient $\mu$.

This behavior is consistent with the physical interpretation of the system:
...
We notice that the algae concentration converges to a limit in each layer. As supposed earlier, 
the vector of concentrations in the column corresponding to the paddle converges to an eigen vector of the matrix. This phenomenon has been observed
with different initial conditions, and with $N$ varying from $3$ to $7$.

\section{Structural Insights from Permutation-Based Optimization}

\subsection{Emerging Patterns in Optimal Permutations}

The exhaustive optimization over permutation matrices reveals consistent structural patterns 
across all tested configurations and system sizes.

In all cases, the optimal permutations exhibit a strong tendency to transport biomass toward 
layers associated with higher growth rates. More precisely, the permutation matrices maximizing 
the objective function $J(P)$ tend to map a large fraction of the biomass trajectories toward 
the subset of layers where the growth coefficient $\mu$ is maximal.
...
\subsection{Connection with the Flashing Light Effect}

The observed structure of optimal permutations can be directly related to the flashing light effect.

As discussed previously, the photosynthetic response is nonlinear with respect to light intensity. 
In particular:
\begin{itemize}
...
Permutation-based mixing induces structured temporal alternations between different light environments. 
In particular, optimal permutations tend to maximize the time spent in high-growth layers while still 
maintaining temporal variability.

This suggests that the optimization process naturally balances two competing effects:
\begin{itemize}
    \item maximizing exposure to favorable conditions,
    \item avoiding constant exposure that would reduce efficiency.
\end{itemize}

\subsection{Extremal Nature of Optimal Strategies}
...
The previous observations lead to the following heuristic explanation:

\begin{quote}
Optimal mixing strategies maximize the temporal variability of light exposure 
while ensuring frequent visits to high-growth regions.
\end{quote}

Permutation matrices naturally achieve this by:
\begin{itemize}
    \item enforcing structured, periodic trajectories,
    \item avoiding diffusive spreading of biomass,
    \item concentrating exposure in favorable layers.
\end{itemize}

This provides a qualitative explanation for the empirical observation that optimal solutions 
are consistently found among permutation matrices.

\subsection{Implications for the Continuous Optimization Problem}

These structural insights strongly suggest that the relaxation of the optimization problem 
to larger sets (stochastic or bistochastic matrices) may not lead to better solutions.

Instead, it is expected that optimal solutions remain located near the extremal points 
of the admissible set.

This hypothesis will be investigated in the following sections using gradient-based optimization 
methods over continuous matrix spaces.
...
\subsubsection{optimal stochastic matrix}
...
The previous optimal permutation matrix algorithm allows us to compare the best permutation matrix to the bistochastic case, and to check if a bistochastic relaxation leads to a higher performance.
In fact, what was shown by the simulations, is that the optimal permutation matrices are at least a local maximum of the objective function on the set of bistochastic matrices. Indeed, launching the optimization algorithm 
with a starting point at the optimal permutation matrix leads to a plateau. This implies that no better bistochastic matrix near the permutation matrix can be found. 

L’ensemble admissible étant convexe, ses points extrémaux (matrices de permutation) jouent un rôle particulier. Nos résultats numériques et analytiques suggèrent que la structure du fonctionnel favorise ces points extrémaux, en cohérence avec l’interprétation physique du flashing effect
\section{Flashing effect}

We have observed a phenomenon of higher growth as the basin's length lowers. For the same parameters, excepted the length L, the algae tend to grow more when the number of mixes increases. 
This can be explained by the fact that if each "layer", for the same total growing time T, gets to each position a larger number of times. 
