...
\begin{itemize}
    \item Configuration 1 (biomass initially in layer \#1):\\
        ...
        \]
        This matrix induces a permutation of layers which induces the allocation of layer 1 in position 2.
...
Here is a graphic representing the total algae mass in each layer as a function of time, for configuration 3 and using the mixing matrix $\mathbf{P}_3$.
...
\[
T = \text{300 s}, \quad u = \text{0.1 m.s^{-1}}.
\]
...
Therefore, it allows us to divide the water pond into a larger number of layers. 

For $N = \text{10}$, the optimization yields the following matrix:
...
do not lead to an increase in the objective function.
This suggests that, for a given permutation matrix $P^\star$, there exists a neighborhood 
$\mathcal{U} \subset \mathcal{B}_N$ such that:
\[
J(P) \leq J(P^\star), \quad \forall P \in \mathcal{U}.
\]

In other words, permutation matrices behave as locally optimal solutions 
...
\[
J_{\text{bistochastic}} \leq J_{\text{permutation}} \quad \text{(observed numerically)}.
\]
...
From this perspective, stochastic or bistochastic mixing can be interpreted as a probabilistic 
